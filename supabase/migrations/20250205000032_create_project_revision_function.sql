...
BEGIN
  ...
  ELSE
    ...
    FROM public.projects
    WHERE id = source_project_id
       OR parent_project_id = source_project_id;
  END IF;
  
  -- Mark old current version as not current
  UPDATE public.projects
  SET is_current_version = false
  WHERE (id = source_project_id OR parent_project_id = COALESCE(v_source_project.parent_project_id, source_project_id))
  ...
  BEGIN
    ...
    ELSE
      v_parent_project_id := source_project_id;
    END IF;
