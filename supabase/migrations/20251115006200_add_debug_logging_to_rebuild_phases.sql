-- Add debug logging to rebuild_phases_json_from_project_phases to diagnose empty operations
-- This will help us understand why operations aren't being found
...
BEGIN
  ...
  LOOP
    ...
    -- Debug: Count operations for this phase
    DECLARE
      op_count_by_phase_id INTEGER := 0;
      op_count_by_standard_phase_id INTEGER := 0;
      op_count_from_standard_foundation INTEGER := 0;
    BEGIN
      SELECT COUNT(*) INTO op_count_by_phase_id
      FROM public.template_operations op
      WHERE op.project_id = p_project_id
        AND op.phase_id = phase_record.id;

      SELECT COUNT(*) INTO op_count_by_standard_phase_id
      FROM public.template_operations op
      WHERE op.project_id = p_project_id
        AND phase_record.is_standard = true 
      ...
      SELECT COUNT(*) INTO op_count_from_standard_foundation
      FROM public.template_operations op
      WHERE phase_record.is_standard = true
        AND phase_record.standard_phase_id IS NOT NULL
      ...
      RAISE NOTICE '🔍 Phase: % (id=%), operations: phase_id=% standard_phase_id=% standard_foundation=%', 
        phase_record.name, phase_record.id, op_count_by_phase_id, op_count_by_standard_phase_id, op_count_from_standard_foundation;
    END;
