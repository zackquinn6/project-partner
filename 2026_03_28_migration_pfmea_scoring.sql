...
-- Detection: failure mode vs cause; measurement robustness; visual vs automated (4 text columns + type + score)
...
('detection', 7, 'Subtle failure mode; easy to miss without detailed examination.', 'Cause hidden (inside wall, interstitial).', 'Fine grout cracks; slight nail pop pattern; HRV imbalance without hood test. Intermittent GFCI trip only under load mix.', 'Detailed visual or functional test—typical D 7+ for subtle defects.'),
