-- is_template is not used; all rows in projects are considered templates.
-- Redefine create_project_revision_v2 so it does not reference is_template.
...
COMMENT ON FUNCTION public.create_project_revision_v2(uuid, text) IS
  'Creates a new project revision from a source project. All projects are templates; is_template is not used.';
