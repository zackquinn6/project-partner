-- CRITICAL FIX: Drop the OLD function signature that's causing the error
-- There are TWO versions of the function - we need to drop the old one first!
...
DROP FUNCTION IF EXISTS public.create_project_with_standard_foundation_v2(text, text);
DROP FUNCTION IF EXISTS public.create_project_with_standard_foundation_v2(text, text, text);
...
-- ============================================
-- STEP 5: Recreate essential triggers
-- ============================================
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'SUCCESS: Only one function version exists. Old function dropped.';
  END IF;
END;
$$;
