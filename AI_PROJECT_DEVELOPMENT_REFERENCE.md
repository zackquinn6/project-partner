...
Living changelog for **guide and planning-standard rule changes only**. Append a row when a durable field, constraint, authoring rule, or SQL lesson changes how future work must be done - and fold that rule into the matching section above. Do **not** edit this file (or the planning standard) merely because you used the guide to author or ship catalog content, migrations, or project build-outs; those land in SQL/migrations and the final report, not here.

| Date | Change | Why |
| ---- | ------ | --- |
| 2026-09-24 | Revision history + `.cursor/rules/ai-project-dev-guide.mdc` continuous improvement: update the guide only when durable rules/fields change; do not append history for ordinary catalog content or project build-out migrations | Using the guide to create content was incorrectly treated as a guide update |
