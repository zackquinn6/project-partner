-- Fix revision creation and remove status column from projects

-- First, update create_project_revision_v2 to properly handle standard vs custom phases
...
BEGIN
  ...
  -- Create new project (revision) - REMOVED status column
  ...
  LOOP
    ...
    FOR operation_record IN
      ...
      ORDER BY display_order
    LOOP
      -- Properly handle standard vs custom phase fields
      INSERT INTO template_operations (
        ...
        new_project_id,
        new_phase_id,
        CASE WHEN phase_record.is_standard THEN phase_record.standard_phase_id ELSE NULL END,
        CASE WHEN NOT phase_record.is_standard THEN phase_record.name ELSE NULL END,
        CASE WHEN NOT phase_record.is_standard THEN phase_record.description ELSE NULL END,
        CASE WHEN NOT phase_record.is_standard THEN phase_record.display_order ELSE NULL END,
...
-- Update create_project_with_standard_foundation_v2 to remove status column
...
BEGIN
  -- Create new project - REMOVED status column
  INSERT INTO projects (
    name,
    description,
    category,
...
-- Remove status column from projects table (only applies to project_runs)
ALTER TABLE projects DROP COLUMN IF EXISTS status;

-- Update projects_status_check constraint to only check publish_status
