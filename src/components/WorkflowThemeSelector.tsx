...
export function WorkflowThemeSelector({
  projectRunId
}: WorkflowThemeSelectorProps) {
  const [open, setOpen] = useState(false);
  const [themeMode, setThemeMode] = useLocalStorage<ThemeMode>(projectRunId ? `workflow-theme-mode-${projectRunId}` : 'workflow-theme-mode', 'light');
  const [colorScheme, setColorScheme] = useLocalStorage<ColorScheme>(projectRunId ? `workflow-color-scheme-${projectRunId}` : 'workflow-color-scheme', 'default');
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setThemeMode('light')} className={`flex items-center gap-3 p-4 rounded-lg border-2 transition-all ${themeMode === 'light' ? 'border-primary bg-primary/5' : 'border-muted hover:border-muted-foreground/50'}`}>
              ...
              <button onClick={() => setThemeMode('dark')} className={`flex items-center gap-3 p-4 rounded-lg border-2 transition-all ${themeMode === 'dark' ? 'border-primary bg-primary/5' : 'border-muted hover:border-muted-foreground/50'}`}>
                <Moon className="w-5 h-5" />
                <div className="text-left">
                  <div className="font-medium text-black">Dark</div>
