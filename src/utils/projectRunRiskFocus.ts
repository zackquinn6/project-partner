...
/** Dashboard label: strip "Risk Radar: " / legacy "Risk Focus: " / "Risk-Less: " prefix from stored name when run is risk-focus. */
export function getRiskFocusAwareDisplayName(run: ProjectRun): string {
  ...
  const stripped = raw.replace(/^(Risk Focus|Risk-Less|Risk Radar):\s*/i, '').trim();
