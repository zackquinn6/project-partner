...
--
-- Note: the app calls create_draft_project_revision instead; prefer:
--   RUN_IN_SUPABASE_create_draft_project_revision.sql (no DROP of old name).
--
-- Fixes create_project_revision when the API returns:
--   23502 null value in column "name" of relation "projects"
-- and failing rows show publish_status "published" for the new row (old function).
--
-- Requires public.rebuild_phases_json_from_project_phases(uuid) to exist.
-- If CREATE fails, run the full file instead:
--   2026_04_01_migration_revision_pfmea_triggers_and_overloads.sql
-- =============================================================================

DROP FUNCTION IF EXISTS public.create_project_revision(uuid, text) CASCADE;

CREATE FUNCTION public.create_project_revision(
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'create_project_revision: source project not found: %', p_source_project_id;
  END IF;

  IF new_name IS NOT NULL AND btrim(new_name) <> '' THEN
    v_revision_name := btrim(new_name);
  ELSE
    v_revision_name := v_src.name;
  END IF;
  IF v_revision_name IS NULL THEN
    RAISE EXCEPTION 'create_project_revision: could not resolve name (new_name empty and source project name is null)';
  END IF;
  ...
  LOOP
    ...
    IF v_new_phase_id IS NULL THEN
      RAISE EXCEPTION 'create_project_revision: phase_operations % references unmapped phase %',
  ...
  LOOP
    ...
    IF v_new_op_id IS NULL THEN
      RAISE EXCEPTION 'create_project_revision: operation_steps % references unmapped operation %',
  ...
  LOOP
    ...
    IF v_new_step_id IS NULL THEN
      RAISE EXCEPTION 'create_project_revision: pfmea_failure_modes % references unmapped operation_step %',
  ...
  LOOP
    ...
    IF v_new_fm_id IS NULL THEN
      RAISE EXCEPTION 'create_project_revision: pfmea_potential_effects % unmapped failure_mode %',
  ...
  LOOP
    ...
    IF v_new_fm_id IS NULL THEN
      RAISE EXCEPTION 'create_project_revision: pfmea_potential_causes % unmapped failure_mode %',
  ...
  LOOP
    ...
    IF v_old_fm_for_ctrl IS NULL THEN
      RAISE EXCEPTION 'create_project_revision: pfmea_controls % has no failure_mode_id and no resolvable cause',
        r_ctrl.id;
    END IF;
    v_new_fm_for_ctrl := (fm_map ->> v_old_fm_for_ctrl::text)::uuid;
    IF v_new_fm_for_ctrl IS NULL THEN
      RAISE EXCEPTION 'create_project_revision: pfmea_controls % unmapped failure_mode %',
  ...
  LOOP
    ...
    IF v_new_fm_id IS NULL THEN
      RAISE EXCEPTION 'create_project_revision: pfmea_action_items % unmapped failure_mode %',
...
COMMENT ON FUNCTION public.create_project_revision(uuid, text) IS
  'Draft revision: copy projects + workflow + PFMEA + risks; phases from rebuild_phases_json_from_project_phases.';
...
GRANT EXECUTE ON FUNCTION public.create_project_revision(uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_project_revision(uuid, text) TO service_role;
