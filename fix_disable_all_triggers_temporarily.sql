-- NUCLEAR OPTION: Temporarily disable ALL triggers during project creation
-- This creates a wrapper function that disables triggers, creates the project, then re-enables them
...
-- ============================================
-- STEP 2: Create the function WITHOUT any trigger dependencies
-- ============================================
...
-- ============================================
-- STEP 3: Recreate ONLY the safe triggers (after project creation is done)
-- ============================================
...
-- Recreate only the safe triggers (NOT cascade_standard_phase_updates_on_operations)
...
BEGIN
  -- Count all triggers on template_operations
  SELECT COUNT(*) INTO trigger_count
  FROM information_schema.triggers
  WHERE event_object_table = 'template_operations'
    AND event_object_schema = 'public';
  
  -- Check for the problematic trigger
  SELECT COUNT(*) INTO problematic_trigger_count
  FROM information_schema.triggers
  WHERE event_object_table = 'template_operations'
    AND trigger_name = 'cascade_standard_phase_updates_on_operations';
  
  RAISE NOTICE 'Total triggers on template_operations: %', trigger_count;
  RAISE NOTICE 'Problematic trigger (cascade_standard_phase_updates_on_operations) count: %', problematic_trigger_count;
  ...
  ELSE
    RAISE NOTICE 'SUCCESS: All problematic triggers removed. Function updated. Safe triggers recreated.';
  END IF;
END;
$$;
