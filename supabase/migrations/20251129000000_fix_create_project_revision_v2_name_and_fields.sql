-- Fix create_project_revision_v2 to:
-- 1. NOT add revision number to project name
-- 2. Copy all project fields including images, project info, etc.
...
BEGIN
  ...
  -- CRITICAL: Copy ALL fields from source project, including images and project info
  -- CRITICAL: Do NOT modify the name - keep it exactly as the source project
  ...
  SELECT 
    name,  -- Keep name exactly as source - do NOT add revision number
    description,
    category,
    'draft',
    created_by,
...
'Creates a new project revision. Preserves project name (no revision number added), copies all project fields including images and project info. Reads position_rule and sets position_value correctly. Includes source tracking for incorporated phases.';
