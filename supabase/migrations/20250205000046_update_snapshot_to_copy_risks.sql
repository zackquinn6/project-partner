-- Migration: Update create_project_run_snapshot to copy template risks to project run
-- ============================================
...
BEGIN
  ...
  PERFORM public.copy_template_risks_to_project_run(
    v_run_id::UUID,
    p_template_id::UUID
  );
...
'Creates IMMUTABLE snapshot of project template. CRITICAL: Resolves dynamic links (isLinked: true) by fetching operations/steps from source_project_id. Also copies template risks to project run. The snapshot contains fully resolved data at the moment of creation - it will NOT change if the standard project is later updated.';
