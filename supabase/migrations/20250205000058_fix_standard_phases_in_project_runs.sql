-- =====================================================
-- FIX STANDARD PHASES IN PROJECT RUNS
-- Ensures that standard phases are correctly identified
-- in project templates and project runs
-- =====================================================

-- Step 1: Update all standard phases in project templates to have is_standard = true
-- This ensures that when phases are copied from Standard Project Foundation,
-- they maintain the is_standard flag
-- Note: We identify standard phases by checking if they came from the Standard Project Foundation
...
-- Step 2: Update create_project_run_snapshot to ensure isStandard is set correctly
-- The function already sets isStandard from pp.is_standard, but let's verify
-- and add a fallback to check phase names for standard phases
...
BEGIN
  ...
  -- CRITICAL: Ensure isStandard flag is set correctly (from is_standard column or phase name)
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', pp.id,
      'name', pp.name,
  ...
  INSERT INTO project_runs (
    ...
    v_start_date,
    v_plan_end_date,
    v_template_phases, -- RESOLVED snapshot - links are dereferenced, isStandard is set
    '[]'::jsonb,
    0,
    'active',
    NOW(),
...
COMMENT ON FUNCTION create_project_run_snapshot IS 'Creates an immutable project run snapshot with complete phases JSONB. Ensures isStandard flag is correctly set from the is_standard database field.';

-- Success message
DO $$
BEGIN
  RAISE NOTICE '✅ Updated standard phases in project templates and fixed create_project_run_snapshot to ensure isStandard flag is set correctly';
END $$;
