...
export function usePartnerAppSettings() {
  ...
  const fetchSettings = async () => {
    try {
      const keys = ['partner_apps_enabled', 'expert_support_enabled'];
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (key === 'partner_apps_enabled' || key === 'expert_support_enabled') fetchSettings();
        }
      )
      .subscribe();
  ...
  return { partnerAppsEnabled, expertSupportEnabled, loading, refetch: fetchSettings };
}
