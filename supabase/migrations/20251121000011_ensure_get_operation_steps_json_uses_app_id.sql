-- Ensure get_operation_steps_json uses app_id instead of app_key
-- This migration explicitly fixes the function to prevent app_key errors
-- The function is called by rebuild_phases_json_from_project_phases which is used by project_templates_live view
  ...
  LOOP
    ...
    IF step_record.apps IS NOT NULL AND jsonb_typeof(step_record.apps) = 'array' THEN
      FOR i IN 0..jsonb_array_length(step_record.apps) - 1 LOOP
        ...
        -- CRITICAL: Use app_id, NOT app_key (app_key does not exist)
...
COMMENT ON FUNCTION public.get_operation_steps_json IS 'Builds step JSON with dynamically linked app names from app_overrides table. Uses app_id (not app_key) to match app_overrides schema.';
